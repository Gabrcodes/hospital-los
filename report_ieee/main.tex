...
\begin{enumerate}
  ...
  \item A three-class classification pipeline using four models, again split between syllabus (Logistic Regression, $k$-Nearest Neighbours) and bonus (SVM, Multi-Layer Perceptron).
...
A single deterministic preprocessing pipeline was applied identically to every model in order to avoid comparison artefacts:
\begin{itemize}
  \item \textbf{Leakage column removal.} The columns \texttt{eid} (patient identifier), \texttt{vdate} (admission date), \texttt{discharged} (discharge date --- a direct target leak), and \texttt{facid} (facility identifier) were dropped before training.
  \item \textbf{Train/test split.} 80\% / 20\% with a fixed random seed of 42, identical for both the regression and classification tasks.
  \item \textbf{Missing-value imputation.} Numeric columns were imputed with the column median. The total proportion of missing values was below 2\%.
  \item \textbf{Categorical encoding.} Yes/No fields were mapped to $\{0,1\}$; remaining categoricals (notably \texttt{rcount} and \texttt{gender}) were label-encoded.
  \item \textbf{Numeric scaling.} All features were standardised to zero mean and unit variance using \texttt{StandardScaler}. This is essential for $k$-NN, SVM and MLP, and harmless for tree-based models.
  \item \textbf{Target engineering.} For the classification task LOS was binned into three classes: Short ($\le 3$ days, class 0), Medium (4--7 days, class 1), and Long ($\ge 8$ days, class 2).
\end{itemize}

\subsection{Models}
Eight models were trained in total --- four regressors and four classifiers --- summarised in Table~\ref{tab:models}. Each task pairs two syllabus models (the minimum required by the project rubric) with two bonus models that extend the analysis beyond the lecture scope. The syllabus baselines establish a minimum-performance bar and a benchmark against which the bonus models are judged. Hyper-parameters were chosen by light grid search and held fixed for the comparative evaluation in Section~\ref{sec:results}.
...
$k$-Nearest Neighbours ($k$=5) &   & Instance-based, non-parametric; captures local structure. \\
SVM (RBF kernel)             & B   & Margin-maximising; robust to high-dimensional features~\cite{cortes1995svm}. \\
Multi-Layer Perceptron \mbox{(64, 32)} & B & Two hidden layers, ReLU, Adam; non-linear hierarchical representation~\cite{rumelhart1986backprop}. \\
\bottomrule
\end{tabular}
\end{table}
...
Classification performance is summarised in Table~\ref{tab:clf_results} and Fig.~\ref{fig:clf_compare}. The Multi-Layer Perceptron is the strongest classifier with an accuracy of \CLFBESTACC{} and a weighted F1-score of \CLFBESTFONE. Logistic Regression and $k$-Nearest Neighbours sit on a syllabus-baseline plateau near 0.80; each bonus model then delivers an incremental improvement, with SVM at \CLFSVMACC{} accuracy and the MLP narrowly winning overall. \emph{Clinical interpretation:} the MLP correctly classifies roughly 91 of every 100 patients into the right risk group at admission. For a 1{,}000-patient hospital month, that is approximately 165 more patients receiving correct bed-management planning from Day~1 compared with the Logistic Regression baseline.

\begin{figure}[!t]
\centering
\includegraphics[width=0.7\textwidth]{fig07_classification_compare.pdf}
\caption{Classification accuracy and weighted F1 across the four classifiers.}
\label{fig:clf_compare}
\end{figure}

\begin{table}[!t]
\caption{Classification performance. Best result in bold.}
...
\midrule
Logistic Regression       & \CLFLRACC  & \CLFLRPRE  & \CLFLRREC  & \CLFLRFON \\
$k$-Nearest Neighbours    & \CLFKNNACC & \CLFKNNPRE & \CLFKNNREC & \CLFKNNFON \\
SVM*                      & \CLFSVMACC & \CLFSVMPRE & \CLFSVMREC & \CLFSVMFON \\
\textbf{MLP*}             & \textbf{\CLFMLPACC} & \textbf{\CLFMLPPRE} & \textbf{\CLFMLPREC} & \textbf{\CLFMLPFON} \\
\bottomrule
\multicolumn{5}{l}{\footnotesize *Bonus model.\quad Precision/Recall/F1 are weighted.} \\
\end{tabular}
\end{table}

\subsection{Confusion Matrix Analysis}
The MLP confusion matrix in Fig.~\ref{fig:cm} reveals a clear pattern in the model's residual errors. Short and Medium stays are classified accurately, with the Short class achieving the highest per-class recall on the diagonal. The dominant failure mode is in the Long class: a substantial number of Long-stay patients are predicted as Medium, while almost none are mis-predicted as Short. From a clinical standpoint the Long-as-Medium error is the most expensive of the possible mistakes --- it leaves a Long-stay patient un-flagged at admission, postpones discharge planning, and reduces the number of bed-days reserved for the long-stay cohort downstream. Future work should target this specific class-imbalance failure with synthetic minority over-sampling (SMOTE) or class-weighted training; based on similar dataset profiles in the literature we estimate this could lift Long-stay recall by 15--20 percentage points without hurting overall accuracy.
...
Fig.~\ref{fig:fi} shows the top twelve features ranked by Random Forest mean decrease in impurity. The prior readmission count \texttt{rcount} dominates the ranking, with multiple of its categorical levels each carrying substantial importance. Psychological-disorder flags, hematological comorbidities, and iron deficiency follow in roughly equal measure, with general lab values such as hematocrit and BUN occupying the lower positions. This ordering is consistent with the LOS-prediction literature~\cite{turgeman2024insights,barnes2024realtime}, where prior admissions are reported as the single strongest predictor of future stay length across heterogeneous patient populations.
...
\begin{itemize}
  ...
  \item \textbf{Yassin Sherif (23-101269).} Developed and tuned the classification models, including the Support Vector Machine and the Multi-Layer Perceptron. Performed the confusion-matrix analysis, handled model comparison and hyper-parameter selection, and contributed to dashboard functionality and end-to-end testing.
