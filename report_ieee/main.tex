...
\subsection*{Conversation Screenshots}

\begin{figure}[!h]
\centering
\includegraphics[width=0.95\textwidth]{ai_session_01.png}
\caption{AI session 1 (Claude). Deployment progress check during the AWS rollout: live URL verification, headline numbers from the new pipeline run, and confirmation that the Streamlit ``Patient Predictor'' tab now drives real joblib-loaded inference. Output was reviewed and the deployment workflow was confirmed by the team.}
\label{fig:ai01}
\end{figure}

\begin{figure}[!h]
\centering
\includegraphics[width=0.95\textwidth]{ai_session_02.png}
\caption{AI session 2 (Claude). Editorial feedback round on the technical report: the team flagged that the report was ``too wordy, very crowded and messy'' and asked for a tightening pass. Claude's diagnosis (caption-then-paragraph repetition, 6-objective lists, oversized conclusion scaffolding) was used as a checklist; the actual rewrites and final wording were authored by the team.}
\label{fig:ai02}
\end{figure}

\begin{figure}[!h]
\centering
\includegraphics[width=0.95\textwidth]{ai_session_03.png}
\caption{AI session 3 (Claude). Discussion of report layout options: the team asked whether two-column IEEE conference style was the right choice for a course technical report. Claude summarised the trade-offs (float drift, narrow columns crushing tables); the final decision to switch to a single-column \texttt{article} layout with IEEE bibliography was made by the team.}
\label{fig:ai03}
\end{figure}

\begin{figure}[!h]
\centering
\includegraphics[width=0.95\textwidth]{ai_session_04.png}
\caption{AI session 4 (Claude). Bibliography validation: the team asked Claude to audit each citation in \texttt{references.bib} against real publications. The audit identified two fabricated references in an earlier draft (a 2024 BMC ``Barnes et al.''\ that did not exist, and a 2024 \emph{Applied Sciences} attribution to Turgeman et al.\ that was actually authored by Almeida et al.). All replacements were verified by the team via web search before being committed to \texttt{references.bib}.}
\label{fig:ai04}
\end{figure}
